...
The **Apollo510B operates as the I2S slave**, while the ADAU1777 operates as the I2S master, generating BCLK and LRCLK/WS. Audio data is transferred in both directions using DMA with ping-pong buffering and explicit cache-coherency handling.
...
- Apollo510B operating as **I2S slave**
- ADAU1777 operating as **I2S master**
...
The linker script supports selectable NVM, ITCM/DTCM and Shared SRAM layouts.
The following default selections are used by the current project:

```text
NVM_SELECT=1
TCM_SELECT=2
SHARED_SRAM_SELECT=3
```
With these default selections, the application uses the following internal
memory regions:
...
In the AmbiqSuite startup source used by the project (`startup_gcc.c`):
...
For this project the codec is configured as a 24 kHz stereo I2S master. AIN2/AIN3 are converted by ADC2/ADC3 and routed through the output ASRCs to the serial output. The serial input is routed through the input ASRCs directly to DAC0/DAC1, allowing the DSP core to remain disabled.

The tested hardware configuration uses the 24.576 MHz external MCLK provided to the ADAU1777. ADAU1777 generates the I2S BCLK and LRCLK/WS signals; the Apollo510B operates as the serial-port clock slave.
...
the ADAU1777 I2S configuration and Apollo510B configuration must remain consistent.
...
The repository contains a small amount of Ambiq-originated source code in
`src/i2s_loopback.c`, which is derived from the AmbiqSuite SDK 5.2.0 I2S
loopback example and has been substantially modified for this project.

The build also uses AmbiqSuite SDK components, including Apollo510 startup,
runtime, HAL, BSP and utility support, from the local SDK installation.
These SDK components are not distributed with this repository.
